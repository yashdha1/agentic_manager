...
async def sales_analyze_product_sales_time(
	...
	Args:
		...
		compare_previous_period: If True, also compute metrics for previous period of same length.
...
async def sales_analyze_products_stats() -> dict:
	...
	Returns:
		Product count, active/inactive split, category count, brand count and price range summary.
	"""
	try:
		async with get_async_session() as session:
			stmt = select(
